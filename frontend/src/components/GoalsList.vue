<template>
    <div class="mx-4">
      <div class="w-full p-4">
        ...
        <div v-if="goals.filter(goal => !goal.is_group_goal && !goal.completed).length > 0" class="grid grid-cols-4 md:grid-cols-4 gap-4">
          <div v-for="goal in goals.filter(goal => !goal.is_group_goal && !goal.completed)" :key="goal.id" class="bg-white rounded-lg shadow p-4 flex flex-col h-56">
            <div class="flex-grow">
      ...
      <div class="w-full p-4">
        <h2 class="text-2xl font-semibold mb-4">Completed Goals</h2>
        <div v-if="goals.filter(goal => goal.completed).length > 0" class="grid grid-cols-4 md:grid-cols-4 gap-4">
          <div v-for="goal in goals.filter(goal => goal.completed)" :key="goal.id" class="bg-white rounded-lg shadow p-4 flex flex-col h-56">
            <div class="flex-grow">
              <h3 class="text-lg font-semibold">{{ goal.title }}</h3>
              <p>{{ goal.description }}</p>
              <span v-if="goal.deadline">Deadline: {{ new Date(goal.deadline).toLocaleDateString() }}</span>
        ...
        <div v-else class="text-center">
          <p>No completed goals found. Start by creating one!</p>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      async toggleGoalCompletion(goal) {
        if (goal.completed) {
        // If the goal is already completed, re-create it as a new goal (re-doing it)
            const newGoal = {
                title: goal.title,
                description: goal.description,
                deadline: goal.deadline,  // Keep the same deadline
                group_id: goal.is_group_goal ? goal.group_id : null,  // Assign group_id if it's a group goal
            };

            try {
                // Create a new goal with the same information
                await axios.post('/goals', newGoal);
                this.fetchGoals();  // Refresh the goals list
            } catch (error) {
                console.error("Error re-creating goal:", error);
            }
            } else {
